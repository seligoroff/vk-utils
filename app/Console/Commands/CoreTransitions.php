...
class CoreTransitions extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->displayIuvya($transitions, $stableCore);
    ...
    private function displayIuvya(array $t, array $stableCore): void
    {
        $this->newLine();
        $this->info('=== ИУВЯ (Индекс Устойчивости Ядра и Воспроизводства) ===');
        $this->newLine();

        // Удержание: доля ядра, оставшаяся в ядре (только core→core vs core→open + core→absent)
        // core→hidden исключён — это не выход из ядра, а изменение приватности
        $coreRetained = $t['core_to_core'];
        $coreExited = $t['core_to_open'] + $t['core_to_absent'];
        $totalCoreEvents = $coreRetained + $coreExited;
        $retention = $totalCoreEvents > 0 ? $coreRetained / $totalCoreEvents : 0;

        // Рост: только open→core (hidden→core — не новобранцы, а изменение приватности)
        $recruits = $t['open_to_core'];
        $lastStable = !empty($stableCore) ? end($stableCore)['stable_core_count'] : 1;
        $growth = $lastStable > 0 ? $recruits / $lastStable : 0;

        // ИУВЯ
        $iuvya = $retention * (1 + $growth);

        $this->line("Удержание ядра: {$coreRetained} / {$totalCoreEvents} событий = " . round($retention * 100, 1) . '% (исключая core→hidden как смену приватности)');
        $this->line("Рост ядра: {$recruits} новобранцев (open→core) / {$lastStable} уст. ядро = " . round($growth * 100, 1) . '% (исключая hidden→core)');

        $verdict = match (true) {
            $iuvya > 0.5 => '✅ здоровое: ядро удерживается и растёт',
            $iuvya >= 0.2 => '⚠️ стабильное: удержание есть, рост слабый',
            default => '❌ деградация: ядро размывается',
        };

        $this->line("ИУВЯ: " . round($iuvya, 3) . " — {$verdict}");
    }
}
