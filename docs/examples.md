...
Начиная с версии 0.9.1, ошибки VK API (таймаут, ограничение частоты, отказ в доступе) корректно отличаются от пустой стены:

```bash
# Таймаут — команда автоматически повторяет запрос (до 2 попыток всего)
php artisan vk:posts-get --owner=-12345678 --from=2026-06-01

# При постоянной ошибке — ненулевой код и безопасное сообщение
echo $?  # 1

# Настройка таймаутов (по умолчанию 30 с)
VK_API_TIMEOUT=60 php artisan vk:posts-get --owner=-12345678 --from=2026-06-01

# При использовании --clear данные не удаляются до успешной загрузки
php artisan vk:posts-get --owner=-12345678 --from=2026-06-01 --db --clear
```

**Важно:** `--clear` выполняет очистку БД только после успешной загрузки всех страниц. Если загрузка прервалась ошибкой, существующие данные в БД сохраняются.
